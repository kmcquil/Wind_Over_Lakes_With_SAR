...
# Find the tile/scene/pass that the buoys are located in the cal/val 1-day orbit and the science 21-day orbit
# List all of the files available for the cal/val 1 day orbit 
# List all of the files available for the science 21 day orbit 
# Find the files of the needed tile/scene/pass
# Check if they have already been downloaded 
# convert to a .tif 

##########################################################################################
##########################################################################################
# Identify the tiles that buoys are located in 
##########################################################################################
##########################################################################################

# Tiles are in kmz format
# Convert the science coverage to a geopandas 
fiona.drvsupport.supported_drivers['KML'] = 'rw'
fiona.drvsupport.supported_drivers['LIBKML'] = 'rw'
science_coverage_file = os.path.join(home, "Data\SWOT_L2_HR_Raster\swot_science_coverage_20240319.kmz")
...
# Convert the cal/val coverage scenes to geopandas and save as a shapefile 
calval_coverage_file = os.path.join(home, "Data\SWOT_L2_HR_Raster\swot_beta_preval_coverage_20231204.kmz")
...
calval_gdf = gpd.read_file(os.path.join(home, "Data\SWOT_L2_HR_Raster\swot_beta_scenes.shp"))
science_gdf = gpd.read_file(os.path.join(home, "Data\SWOT_L2_HR_Raster\swot_science_tiles.shp"))
buoy_info_shp = gpd.read_file(os.path.join(home, "Data/Buoy/buoy_info.shp"))
buoy_info_shp['scene_calval'] = str(np.nan)

# Loop through points to find the scens they are within
...
def check_scene(row, scenes):
    ...
    if ((len(scene) > 0) & (is100 == True)): return scene[0]
    else: return np.nan

# Check if those links are already downloaded 
def check_downloaded(link, folder):
    link = link.split("/")[5]
    check = os.path.isfile(os.path.join(home, "Data/SWOT_L2_HR_Raster", folder, link)) == False # so that we get true values for non-downloaded files
    return check

##########################################################################################
# Start with science orbit
# Create a dt of all potential links for download
file_dt = pd.read_csv(os.path.join(home, "Data/SWOT_L2_HR_Raster/SWOT_L2_HR_Raster_2.0/all_files.txt"))
...
download_mask = file_dt_dwnld['link'].apply(check_downloaded, folder="SWOT_L2_HR_Raster_2.0")
file_dt_dwnld = file_dt_dwnld[download_mask]
file_dt_dwnld['link'].to_csv(os.path.join(home, "Data/SWOT_L2_HR_Raster/SWOT_L2_HR_Raster_2.0/links.txt"), sep='\t', index=False, header=False)
...
# Create a dt of all potential links for download
file_dt = pd.read_csv(os.path.join(home, "Data/SWOT_L2_HR_Raster/SWOT_L2_HR_Raster_1.1/all_files.txt"))
file_dt = file_dt.iloc[:,1:2]
file_dt = file_dt.rename(columns={file_dt.columns[0]:'fp'})
...
# Check if the file has already been downloaded 
file_dt_dwnld["link"] = file_dt_dwnld["fp"].str.split('-', n=1, expand=True).iloc[:, 1]
download_mask = file_dt_dwnld['link'].apply(check_downloaded, folder="SWOT_L2_HR_Raster_1.1")
file_dt_dwnld = file_dt_dwnld[download_mask]
file_dt_dwnld['link'].to_csv(os.path.join(home, "Data/SWOT_L2_HR_Raster/SWOT_L2_HR_Raster_1.1/links.txt"), sep='\t', index=False, header=False)
