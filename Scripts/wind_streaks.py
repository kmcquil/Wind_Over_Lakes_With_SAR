...
df = wdir_subset[["buoy_id", "satellite", "image_id"]]
...
def clip_sig0(buoy_image_id):
    ...
    if satellite == "Sentinel1":
        fp = os.path.join(home, "Data", satellite, "Processed", os.path.basename(row["image_id"].iloc[0]) + ".nc")
        src = rioxarray.open_rasterio(fp)
        sig0 = src.sig0

    if "SWOT" in satellite:
        fp = os.path.join(home, "Data", "SWOT_L2_HR_Raster", satellite, os.path.basename(row["image_id"].iloc[0]) + ".nc")
        src = rioxarray.open_rasterio(fp)
        sig0 = src.sig0
    ...
    sig0_clipped_clipped = sig0_clipped_clipped.where(sig0_clipped_clipped != sig0_clipped_clipped.rio.nodata) # added this post s1 
    sig0_clipped_clipped = sig0_clipped_clipped.squeeze()

    sig0_np = sig0_clipped_clipped.to_numpy()

    f, ax = plt.subplots(figsize=(11, 11))
    sig0_clipped_clipped.plot.imshow(cmap="Greys",
                        ax=ax, vmin=np.nanquantile(sig0_np, 0.1), vmax=np.nanquantile(sig0_np, 0.9))
    buoy_reproj.plot(color='red',
                    edgecolor='red',
                    ax=ax)
